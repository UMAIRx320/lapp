...
def login_lagi334():
	...
	warna = random.choice([
 P, M, H, K, B, U, O, N])
	cetak(nel(f"[bold green]Wannn GANTENG [bold yellow]MEMINTA ANDA UNTUK MEMASUKAN COOKIES KARENA GAK ADA COOKIES GAK BISA LOGIN ANJIM",style="blue bold",title="[bold red]•[bold yellow]•[bold green]•[bold red] informasi [bold green]•[bold yellow]•[bold red]•"))
...
def menu(my_name,my_id):
	...
	print('2. Crack Masal')
	print('3. Exit')
	#print('4. File Token Manual')
	_____WANNN__GANTENG_____ = input('\nChouse : ')
	print('')
	if _____WANNN__GANTENG_____ in ['1']:
		dump_publik()
	elif _____WANNN__GANTENG_____ in ['2']:
		dump_massal()
#	elif _____WANNN__GANTENG_____ in ['3']:
#		crack_file()
	####elif _____WANNN__GANTENG_____ in ['3']:
		###result()
	####elif _____WANNN__GANTENG_____ in ['4']:
		####file_cp()
	elif _____WANNN__GANTENG_____ in ['3']:
...
def dump_publik():
	try:
		...
		exit()
	print('Ketik ME Jika Ingin Crack sendiri')
...
#		method.append('touch')
		...
		method.append('mobile')
	print('')
	pwplus=input('PENGEN PW MANUAL? ( Y/N) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def passwrd():
	global prog,des
	print(f'AKUN {h}OK{x} Save in : {h}OK/%s {x}'%(okc))
	print(f'AKUN {k}CP{x} Save in : {k}CP/%s {x}'%(cpc))
	print(f'Setiap 500 ID ONN/OFF MODE PESAWAT\n')
#	os.system('clear')
